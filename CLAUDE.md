...
- `mac_setup/configs/claude/` — Claude Code config: `settings.json`, `mcp.json`, `claude-notify.sh`, `statusline-command.sh`
...
- **Claude Code** uses `opusplan` model, auto mode, with Stop/Notification hooks wired to `~/.claude/claude-notify.sh`. Config lives in `mac_setup/configs/claude/settings.json`.
