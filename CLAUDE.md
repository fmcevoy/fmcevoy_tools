...
- **Claude Code** runs in auto mode; the model is set in `settings.json`. Hooks (Stop, Notification, SessionStart) are managed by meldr — run `meldr install-hooks` to wire them into `~/.claude/settings.json`, and `meldr doctor hooks` to check them. Notification routing is entirely meldr's: it resolves the pane from the process tree and a live tmux snapshot, so there is no `claude()` shell wrapper and no `MELDR_TMUX_*` variables. Config template lives in `mac_setup/configs/claude/settings.json`.
